...
include { align_dedup } from "./modules/align_dedup.nf" addParams(out_folder: "${params.bam_folder}")
include { QC_bam } from "./modules/qc_bam.nf" addParams(out_folder: "${params.qc_folder}")

include { deepvariant } from "./modules/deepvariant.nf" addParams(out_folder: "${params.vcf_folder}/single_vcf")
include { merge_gvcfs } from "./modules/merge_gvcfs.nf" addParams(out_folder: "${params.vcf_folder}/merged_vcf")
include { process_deepvariant_vcf } from "./modules/process_deepvariant_vcf.nf" addParams(out_folder: "${params.vcf_folder}/processed_vcfs")

include { SV_singlesample } from "./modules/sv_singlesample.nf" addParams(out_folder: "${params.sv_folder}/raw_data")
include { SV_mergesamples } from "./modules/sv_mergesamples.nf" addParams(out_folder: "${params.sv_folder}/merged_vcf")

include { ROH_detection } from "./modules/bcftools_roh.nf" addParams(out_folder: "${params.roh_folder}")
include { ExpansionHunter } from "./modules/expansion_hunter.nf" addParams(out_folder: "${params.exphunter_folder}")
